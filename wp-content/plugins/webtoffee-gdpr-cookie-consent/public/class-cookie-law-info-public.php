        ...
        {       
            ...
            if (@!$term->term_id) {
                wp_insert_term(
                        ...
                        'cookielawinfo-category',
                        array(
                            'description' => 'This is an necessary category.',
                            'slug' => 'necessary'
                        )
                );
            }    
            ...
            if(@!$term->term_id) 
            {
                wp_insert_term(
                        ...
                        'cookielawinfo-category',
                        array(
                            'description' => 'This is an non-necessary category.',
                            'slug' => 'non-necessary'
                        )
                );
            }
	...
	*/
	public function cookielawinfo_inject_cli_script() 
	{
		$the_options = Cookie_Law_Info::get_settings();
	  	...
	  	if ( $the_options['is_on'] == true ) 
	  	{             
			$the_cookie_list =$this->get_cookie_list();	        
		    ...
		    $notify_html = '<div id="' .$this->cookielawinfo_remove_hash( $the_options["notify_div_id"] ) . '" data-cli-geo-loc="'.$geo_loc_enabled.'" style="'.$cli_bar_style.'"><div class="cli-wrapper">'.
		    $cli_close_btn_html.
	...
	public function include_user_accepted_cookielawinfo()
	{
	     ...
	     if($the_options['is_on'] == true && !is_admin()) 
	     {
	        ...
	        {	  
				...
				$scripts = ''; 
				if($cookie['loadonstart']==1)
				{
					$is_script_block = 'false';  
				}   
				$script_load_on_start = 'false';
				if(isset($cookie['loadonstart']) && $cookie['loadonstart']==1){
					$script_load_on_start = 'true';
				}	        
	    ...
	    {	        
	        ...
	        {    
				...
				{     
					...
					{	
						$wt_cli_replace = 'data-cli-class="cli-blocker-script"  data-cli-category="'.$key.'" data-cli-script-type="'.$key.'" data-cli-block="'.$script_load_on_start.'" data-cli-element-position="body"';
