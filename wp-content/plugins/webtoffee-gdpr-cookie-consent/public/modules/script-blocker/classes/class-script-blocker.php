        ...
        try {
            ...
            if($scripts && is_array($scripts)){
                ...
                {    
                    ...
                    {
                        $wt_cli_is_html_element=$v['has_html_elem'];
                ...
                {   
                    ...
                    if(isset($thirdPartyScript[$scriptkey])) {   
                        ...
                        if($v->category_slug!="" || $v->category_slug !== NULL) { //a category assigned
                            ...
                            if($v->loadonstart==1 || $v->cliscript_status == 'no') {
                            ...
                            $placeholder_text = sprintf( esc_html__("Accept <a class='cli_manage_current_consent'>%s</a> cookies to view the content.","cookie-law-info"),$category->name);
            ...
            if ( (preg_match('/<iframe.*(src=\"(.*)\").*>.*<\/iframe>/', $match, $element_match)) ||(preg_match('/<object.*(src=\"(.*)\").*>.*<\/object >/', $match, $element_match)) || (preg_match('/<embed.*(src=\"(.*)\").*>/', $match, $element_match)) || (preg_match('/<img.*(src=\"(.*)\").*>/', $match, $element_match)) ) {
                ...
                if (preg_match("/type=/", $match)) {
                    ...
                    if(!empty($output_array)) {   
                        $match = str_replace($output_array[1], 'type="text/plain"'.' '.$wt_cli_replace, $match);
                    }
        
                }
                else {
                    
                    $match = str_replace('<script', '<script type="text/plain"'.' '.$wt_cli_replace, $match);   
